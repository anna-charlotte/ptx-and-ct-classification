...
from ptx_classification.data.datamodules import DataModuleChestXray14CheXpert
...
def training_function(config: Dict[str, Any]) -> None:
    ...
    early_stop_callback = EarlyStopping(
        monitor=f"AUC/{class_for_early_stopping}/ChestXray14/VAL",
        min_delta=0.00,
        patience=patience,
        verbose=False,
        mode="max",
    ...
    metrics = metrics_to_monitor(datasets=["ChestXray14", "CheXpert"], labels_to_use=labels_to_use)
    ...
    ct_labels_chestxray14: Optional[ChestTubeDataFrame] = None
    ct_labels_chexpert: Optional[ChestTubeDataFrame] = None
    cut_off_chestxray14 = None
    cut_off_chexpert = None

    if ct_labels_path is not None:
        ct_labels_chestxray14 = read_in_csv_as_chest_tube_data_frame(
            path=Path(ct_labels_path) / "predictions" / "prediction_for_ChestXray14Dataset.csv"
        )
        cut_off_chestxray14 = load_json(Path(ct_labels_path) / "youden_index.json")["youden index"]

        ct_labels_chexpert = read_in_csv_as_chest_tube_data_frame(
            path=Path(ct_labels_path)
            / "predictions"
            / "prediction_for_CheXpertDataset_FrontLat-Frontal_APPA-all.csv"
        )
        cut_off_chexpert = load_json(Path(ct_labels_path) / "youden_index.json")["youden index"]

    print(f"cut_off_chestxray14 = {cut_off_chestxray14}")

    batch_size = config["data_modules"]["batch_size"]
    resize = None if "resize" not in config.keys() else config["resize"]
    print(f"resize = {resize}")

    chestxray14_datamodule = ChestXray14DataModule(
        dataset=ChestXray14Dataset(
            root=data_dir,
            cache_dir=cache_dir,
            resize=resize,
            class_labels=labels_to_use,
            ct_labels=ct_labels_chestxray14,
            ct_cut_off=cut_off_chestxray14,
        ),
        batch_size=batch_size,
        data_aug_transforms=data_aug_transforms,
        train_val_split=(
            config["data_modules"]["ChestXray14"]["train_size"],
            config["data_modules"]["ChestXray14"]["val_size"],
        ),
        train_class_weights=config["data_modules"]["ChestXray14"]["train_class_weights"],
        train_num_samples=1000,
    )

    chexpert_datamodule = CheXpertDataModule(
        dataset=CheXpertDataset(
            ...
            resize=resize,
            class_labels=labels_to_use,
            ct_labels=ct_labels_chexpert,
            ct_cut_off=cut_off_chexpert,
        ),
        batch_size=batch_size,
        data_aug_transforms=data_aug_transforms,
        train_val_test_split=(
            config["data_modules"]["CheXpert"]["train_size"],
            config["data_modules"]["CheXpert"]["val_size"],
            config["data_modules"]["CheXpert"]["test_size"],
        ),
        train_class_weights=config["data_modules"]["ChestXray14"]["train_class_weights"],
        train_num_samples=1000,
    )

    datamodule = DataModuleChestXray14CheXpert(chestxray14_datamodule, chexpert_datamodule)
...
def metrics_to_monitor(datasets: List[str], labels_to_use: List[str]) -> Dict[str, str]:
    metrics = {f"loss/{', '.join(datasets)}": "loss"}
    for label in labels_to_use:
        metrics[
            f"AUC {label}/{', '.join(datasets)}/TRAIN"
        ] = f"AUC/{label}/{', '.join(datasets)}/TRAIN"
        for i, dataset in enumerate(datasets):
            metrics[f"loss_val/{dataset}"] = f"loss_val/{dataset}/dataloader_idx_{i}"
...
def main() -> None:
    ...
    ray.init(_temp_dir="/u/home/gerh/ray_tmp")
    tune.run(
        training_function,
        local_dir=local_dir,
        ...
        config={
            ...
            "data_modules": {
                "ChestXray14": {
                    "train_size": ChestXray14Splits.N_TRAIN,
                    "val_size": ChestXray14Splits.N_VAL,
                    "test_size": ChestXray14Splits.N_TEST,
                    "train_class_weights": (1.0, 10.0),
                },
                "CheXpert": {
            ...
            "labels_to_use": [ChestXray14Labels.PTX],
            "class_for_early_stopping": ChestXray14Labels.PTX,
            "pretrained": True,
            "ten_crop_size": tune.grid_search([None]),
            "resize": tune.grid_search([(256, 256)]),
            "lr_scheduler_factor": tune.grid_search([0.1, 0.5]),
            "lr_scheduler_monitor": "loss_val/CheXpert/dataloader_idx_1",
