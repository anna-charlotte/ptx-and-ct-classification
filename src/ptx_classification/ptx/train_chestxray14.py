...
def training_function(config: Dict[str, Any]) -> None:
    ...
    metrics = metrics_to_monitor(dataset="ChestXray14", labels_to_use=labels_to_use)
    ...
    chestxray14 = ChestXray14DataModule(
    ...
    trainer.fit(model=model, datamodule=chestxray14)
    trainer.test(model=model, dataloaders=chestxray14.test_dataloader())
    ...
    tune.run(
        ...
        config={
            ...
            "data_module": {
                ...
                "train_class_weights": tune.grid_search([(1.0, 5.0), (1.0, 10.0)]),
                "batch_size": 7,
            },
            ...
            "lr_scheduler_factor": tune.grid_search([0.1, 0.3, 0.5]),
            "lr_scheduler_monitor": "loss_val/ChestXray14",
